...
from model.custom_exceptions import DestinationNotFound, NoDestinationsRegistered, DestinationAlreadyInSystem, MissingEmergencyContactInfo
...
class Destination_Logic:
    ...
    def register_destination(self, new_destination: Destination):
        ...
        try: #do i need the variable here or could it start from
            destination = self.get_destination(new_destination.airport)
            raise DestinationAlreadyInSystem
            ...
            raise MissingEmergencyContactInfo
        ...
        if destinations_list:
            for destination in destinations_list:
                if destination.airport == airport:
                    return destination
        raise NoDestinationsRegistered
